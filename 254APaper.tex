...
\indent Suppose we are given a polynomial equation over the rationals, the natural question to ask is whether or not there are any solutions.
If our equation is homogeneous, one may reduce to integer solutions, and if one suspects there are none, a natural first direction is to look mod $p^n$ for some prime $p$ (or over $\Q_p$). 
If there are no solutions there we can then conclude there are no "global" solutions.
Otherwise, if this is not possible for any $p$, we may begin to believe that there should be a global solution.
We will show this is indeed the case for quadratic forms (homogeneous degree 2 equations) over any number field $K$ in the Hasse-Minkowski theorem.
Our treatment roughly follows \cite{ATOQF} in that we will need to assume two major results for its proof.
In addition, we will go over the notions of quadratic spaces and quaternion algebras beforehand. 
We also carry over some convenient results from \cite{ACIA} where it is seen fit.\newline

\indent On the other hand, for equations of higher degree there need not be such a local-global principle, one famous example which we cover at the end is Selmer's equation, whose treatment is based off \cite{LFC}. 


\section{Quadratic Spaces}

In this section $K$ will always be a field which is assumed to be of characteristic different from 2. \newline

\indent	A \textit{quadratic form} over a field $K$ is a polynomial in $n$ variables over $K$, which is homogeneous of degree 2.
A quadratic form $f$ can be expressed as 
\[f(X_1, \dots, X_n) = \sum_{i,j = 1}^n a_{ij} X_i X_j = f(X) \]
where $X = (X_1, \dots, X_n)$.
As $\text{char}(K_ \neq 2 $ we may take the averages of $a_{ij}$ and $a_{ji}$ and assume they are equal, so that $f$ determines a unique symmetric matrix $M_f = (a_{ij})$.
In this way, we have in matrix notation that 
\[f(X) = X^T  M_f  X\]

\indent Two quadratic forms $f,g$ are said to be \textit{equivalent} (which we denote as $f \sim g$) if there exists an invertible matrix $C \in \text{GL}_n(K)$ such that $f(X) = g(C  X)$.
Now since 
\[g(C  X) = (C X)^T M_f (C  X) = X^T  (C^T  M_f  C)  X\]
equivalence of quadratic forms amounts to congruence of their associated matrices $M_f$ up to transformations of the form $C^T  M_g  C$ for $C \in \text{GL}_n(K)$.
...
We now give a more geometric way of interpreting quadratic forms (up to equivalence) through quadratic spaces. 
...
It is in fact a \textit{quadratic map} as it satisfies the following two properties, namely 
...
Having defined our objects of interest, we now define our maps of interest between them. 

\begin{definition}[Metric Morphism]
	A \textit{metric morphism}  $\phi: (V,B) \to (V', B')$ between quadratic spaces consists of a linear map $\phi: V \to V'$  such that $\forall x,y \in V$ we have $B'(\phi(x), \phi(y)) = B(x,y)$.
\end{definition}

\begin{remark}
	At least in the case $K = \R$ and the quadratic map $\Q$ is strictly positive away from 0, $B$ defines a metric on $V$ by $|x - y| = \sqrt{Q(x-y)}$, in which case a metric morphism as defined above is equivalent to preserving the metric in the usual sense.
\end{remark}

This naturally defines a category whose objects are quadratic spaces over $K$, and morphisms being metric morphisms (It is straightforward to verify identity maps are metric morphisms, and that they compose). 
If $(V,B)$ and $(V',B')$ are isomorphic in this category (which we denote as $(V,B) \cong (V',B'))$, it is clear from our above correspondence that $(f_B) \cong (f_{B'})$.
...
\begin{definition}[Nondegeneracy]
	For $(V,B)$ a quadratic space, and $M$ the corresponding symmetric matrix to some form in the equivalence class $(f_B)$, then $(V,B)$ is called \textit{nondegenerate} if the following equivalent conditions hold
	\begin{enumerate}
...
\begin{proof}
	...
	\indent For the second result, it suffices to note that $(S^{\perp})){\perp} \subset S$ (under the usual canonical isomorphism $V \cong (V^*)^*$ and since each $s \in S$ is 0 under those functionals in $S^{\perp}$).
...
We first define what it means for a quadratic form to represent a value in $K^{\times}$, an importnat notion for us.
...
The corresponding quadratic map of $V_1 \perp V_2$ satisfies $Q(x_1, x_2) = B((x_1, x_2), (x_1, x_2)) = B_1(x_1, x_2) + B_2(x_1, x_2) = Q_{B_1}(x_1) + Q_{B_2}(x_2)$.
...
\begin{notation}
	We shall denote the $\langle d \rangle$ for the isomorphism class of 1-dimensional quadratic spaces corresponding to the quadratic form $d X^2$.
\end{notation}
...
\begin{proof}
	...
	\indent Conversely, assume that $V$ represents $d$.
	...
	As noted, the quadratic map associated to $V$ are then defined as the composites of those associated to $W$ and $\text{rad}(V)$ (the latter identically zero), so any quadratic form associated to $V$ represents the same values as the on restricted to $W$.
	...
	But we have $\text{dim}(K \cdot v) + \text{dim}((K \cdot v)^{\perp}) = \text{dim}(W)$ from \Cref{prop:nondeg-ss-dim} so therefore 
...
\end{corollary}
 \begin{proof}
	...
	Thus if $V' = \langle d_1 \rangle \perp  \dots\perp \langle d_{n-1} \rangle$ from our induction hypothesis we have that $V \cong \langle d \rangle \perp \langle d_1\rangle \perp \dots \perp \langle d_{n-1} \rangle$ as desired.
 \end{proof}

In terms of quadratic forms the above translates to the following corollary.
 ...
 \begin{notation}
 	We shall denote the quadratic space $\langle d_1 \rangle \perp \dots \perp \langle d_n \rangle$ as $\langle d_1, \dots, d_n \rangle$.
...
Then $y$ is orthogonal to $s$, for any $s_j$ we explicitly verify that 
\begin{align*}
	B(y, s_j) = B(v, s_j) - \sum_{i = 1}^{k} \frac{B(v, s_j)}{B(s_i, s_i)} B(s_i, s_j) = B(v, s_j) -  \frac{B(v, s_j)}{B(s_j, s_j)}B(s_j,s_j) = 0 
\end{align*}
as the $s_i$ are mutually orthogonal. 
Thus $y \in S^{\perp}$ and $v = y + \sum_{i = 1}^{k} \frac{B(v, s_i)}{B(s_i, s_i)} s_i \in S \oplus S^{\perp}$ shows they span $V$ as we want.
\end{proof}

We are now ready to define a major invariant associated to a quadratic form.
...
We note that the discriminant is indeed well-defined in $K^{\times}/ (K^{\times})^2$ (The \textit{group of square classes}) as for any $M_g$ associated to an equivalent quadratic form $g$ we have 
...
Here we introduce in an important quadratic space which will arise a lot in later sections.
 ...
 \begin{theorem}[Hyperbolic Plane]\label{theo:hyperbolic-plane}
	...
	\end{enumerate}
	The isommorphism class of such spaces is called the \textit{hyperbolic plane}, which we denote as $\mb{H}$.
 \end{theorem}
 ...
 \begin{proof}
	...
	But we have $f \sim a Y_1 Y_2$ from \Cref{ex:hyperbolic-plane}, a form which clearly represents any element in $K^{\times}$, in particular 1.
	Now $V \cong \langle 1 \rangle \perp V'$ from \Cref{prop:rep-criterion1}. 
	Then $d(V) = d(V')$ and from our assumption we must have $V' \cong \langle -1 \rangle$ as wanted. \newline
	\indent Finally, to show $3. \to 1.$, note that $V$ is non-degenerate as its corresponding symmetric matrix is invertible. 
	Now $(x, x)$ is an isotropic vector in $\langle 1, -1 \rangle$ for any $x \in K$ so $V$ is isotropic as well, concluding the proof.
 \end{proof}

 \begin{remark}
	 As we outlined in the proof, note that \Cref{ex:hyperbolic-plane} shows that $\mb{H}$ has corresponding equivalence class of quadratic forms containing the form $X_1 X_2$, perhaps justifying its name.
 \end{remark}
 
 \begin{proposition}\label{prop:isotropic-hyperbolic}
	A nondegenerate quadratic space $(V,B)$ is isotropic iff $V$ contains a hyperbolic plane as a subspace, in which case $V$ represents all of $K^{\times}$.
\end{proposition}
...
\begin{proof}
	...
	Then consider $y = 2z - B(z,z)v$. 
	It is linearly independent from $v$ as $z$ is (any multiple of $v$ has $B(kv,v) = kB(v,v) = 0$). 
	Furthermore 
	\begin{align*}
		B(y,y) = B(2z - B(z,z)v,2z - B(z,z)v) &= 4B(z,z) -  4B(z,z) B(v,z) + B(z,z)^2 B(v,v)\\
																&= 4B(z,z) - 4 B(z,z)\\
																&= 0
	\end{align*}
and 
\begin{align*}
	B(y,v) = 2B(z,v) - B(z,z)B(v,v) = 2B(z,v) = 2
\end{align*}
so the quadratic form $f$ corresponding to the subspace $Kv \oplus Ky$ is $4X_1X_2 \sim X_1X_2$ (once again as char $K$ is different from 2) so $V$ contains a hyperbolic plane $\mb{H}$ as claimed. \newline
...
\begin{proposition}\label{prop:represent-prop1}
	A nondegenerate quadratic form $Q$ represents $d \in K^{\times}$ if and only if $Q -dZ^2$ (for a new variable $Z$) is isotropic.
\end{proposition}
\begin{proof}
	We may assume that $Q = d_1 X_1^2 + \dots  + d_n X_n^2$ in which case $Q - dZ^2 = d_1 X_1^2 + \dots  + d_n X_n^2 - dZ^2$.
	If $Q$ represents $d$, so that $Q(x) = d$ for some $x \in K^n$, then 
	\[(Q -dZ^2)(x,1) = Q(x) -d = d - d = 0\]
	shows that $Q - dZ^2$ is isotropic. \newline
\indent Conversely, suppose that $Q - dZ^2$ is isotropic, so that $Q - dZ^2$ evaluated at $(x_1, \dots, x_n, y) = 0$.
...
Arguing along the same lines we obtain a similar result, in this case for the difference of two quadrati forms.
\begin{proposition}
	Let $Q = f -g$ be a quadratic form, for $f,g$ quadratic forms in distinct variables.
	Then $Q$ is isotropic if and only if there exists an $a \in K^{\times}$ such that $f,g$ both represent $a$.
\end{proposition}
 
\begin{proof}
If $f,g$ both represent $a$, then $f(x_1, \dots, x_n) = g(y_1, \dots, y_m) = a$, but then 
\[Q(x_1, \dots, x_n, y_1, \dots, y_m) = f(x_1 \dots, x_n) - g(y_1, \dots, y_m) = a-a = 0\]
so $Q$ is isotropic. 
Conversely, if $Q$ is isotropic, we have 
\[Q(x_1, \dots, x_n, y_1, \dots, y_m) = 0 \Rightarrow f(x_1, \dots, x_n) = g(y_1, \dots, y_m)\]
for some $x_1, \dots, y_m$.
If $f(x_1, \dots, x_n)$ and $g(y_1, \dots, y_m)$ are zero, then one of them must represent zero, so is isotropic and represents any nonzero value the other form represents.
Otherwise, the values are equal to some common $a \in K^{\times}$ proving our claim.
\end{proof}
...
\begin{proposition}\label{discriminant-splitting}
	Let $f$ be a four-dimensional quadratic form over a field $K$ of discriminant $d$.
...
\begin{theorem}\label{theo:geometric-witt-cancel}
	If $(V, B)$ and $(V', B')$ are isomorphic and non-degenerate, then every injective metric morphism $s: U \to V'$ from a subspace $U$ of $V$ can be exteneded to a metric isomorphism of $V$ onto $V'$.
\end{theorem}

\begin{proof}
	We first show we can reduce to the case where $U$ is non-degenerate by showing that in this case $s$ can always be extended to a map $s': U' \to V$, such that $s'|_{U} = s$ and $U'$ contains $U$ and is one-dimension larger.
...
We claim the linear map $\phi: U \oplus Ky \to U' \oplus Ky'$ which is the same as $s$ on $U$ and maps $y$ to $y'$ is our desired one.
...
\[B(x \pm y, x \pm y) =  B(x,x) \pm 2B(x,y) + B(y,y) = 2B(x,x) \pm 2B(x,y)\]
cannot be 0 for both values of $\pm y$ (otherwise one can show that $B(x,x) = 0$) so one of $x \pm y$ is anisotropic. 
Up to replacing $y$ with $-y$ we may assume $x + y$ is anisotropic.
...
We have $U = Ky \perp (Ky)^{\perp}$ by \Cref{prop:nondegen-decomp}. 
From the case as above there exists an automorphism $\sigma$ of $V$ extending the map on $Ky$.
Then $\sigma^{-1} s : U \to V$ is the identity on $Ky$ and since it is metric (being the composite of two metric morphisms) on $U$, it must maps $Ky^{\perp}$ into $V'$, the orthogonal complement of $U$ in $V$. By our inductive hypothesis this extends to a map $\phi': (Ky)^{\perp} \to V$. 
Then the linear map $\varphi$ defined as the identity on $Ky$ and $\phi'$ on $(Ky)^{\perp}$ extends $\sigma^{-1}s$ so $\varphi \circ \sigma$ is an extension of $s$ as desired.
\end{proof}
...
\end{corollary}
\begin{proof}
	If $f \sim f'$ are nondegenerate $n$-ary quadratic forms, then the corresponding quadratic spaces on $K^n$ are isomorphic.
	Then $g \sim g'$ gives an isomorphism from their corresponding subspaces, which from the above theorem extends to an isomorphism between the $K^n$.
...
Now among the $ \frac{p-1}{2}$ quadratic residues in $( \Z /p\Z)^*$, if $\frac{d}{2}$ is among them, say $a^2 = \frac{d}{2}$ then $a^2 + a^2 \equiv \frac{d}{2} +  \frac{d}{2} \equiv d \bmod{p}$.
...
Without the pair consisting of just $\frac{d}{2}$, there are $\frac{p-1}{2}$ pairs, so by the pigeonhole principle there must be one pair $(x,y)$ with both values a square, and the solutions to $a^2 = x, b^2 = y$ then have $a^2 + b^2 \equiv d \bmod{p}$ by construction. \newline
\indent The statement for quadratic forms of rank $\geq 3$ once again reduces to just the case for forms of rank $3$.
If such a quadratic form $f$ is identically zero this is clear otherwise there is a $x \in K^3$ such that $f(x) = d \neq 0$.
But then we have $f \sim f' + dZ^2$ from \Cref{coroll:rep-criterion1}, where $f'$ is of rank 2. 
But as we just showed $f'$ represents all nonzero vaues and hence $-d$, so $f$ must be isotropic as claimed.
\end{proof}

So quadratic forms of rank 2 over $\mathbb{F}_q$ always represent all of $K^{\times}$ (In particular finite fields $\mb{F}_q$), such fields with this property have a nice characterization of their quadratic forms as we show.
...
for $d$ a representative of $K^{\times}/(K^{\times})^2$. 
...
There are analogous constructions of the usual quaternions (over $\R$) to an arbitrary field $K$ of characteristic different from 2, which we give an overview of in this section.
Even more, each naturally has the structure of a quadratic space which we describe.
We will also define the Hilbert symbol, and an equivalence between this symbol taking on the value 1 and certain behavior of an associated quaternion algebra.
...
\begin{proposition}\label{prop:basic-facts}
	We have
\begin{enumerate}
	\item$( \frac{a,b}{K}) \cong ( \frac{ax^2,by^2}{K}), \quad a,b,x,y \in K^{\times}  $
...
\begin{proof}
	...
	However, since $xi, yj \in \qa{a}{b}$ satisfy $(xi)^2 = ax^2, (yj)^2 = by^2$ and $(xi)(yj) = (-yj)(xi)$ (as scalars commute) the map $\qa{a}{b} \to \qa{ax^2}{by^2}$ sending $xi \to i'$ and $yj \to j'$ defines an isomorphism of $K$-algebras (with inverse sending $i' \to i/x, y' \to y/j$). \newline
	\indent For 2.) we set $1 = \begin{bmatrix} 1 & 0 \\ 0 & 1 \end{bmatrix}, i = \begin{bmatrix} 0 & 1 \\ -1 & 0 \end{bmatrix}$ and $j = \begin{bmatrix} 0 & 1 \\ 1 & 0 \end{bmatrix}$
	It is straightforward to verify that it satifies the relations of the quaternion algebra. 
	...
	We know the center $Z(\mb{M}_2(\overline{K}))$ of the $2 \times 2$ matrix algebra are just the scalars $\overline{K} \cdot 1$.
	...
	As $\mb{M}_2(\overline{K}))$ is a simple algebra this is impossible, so $\qa{a}{b}$ is also simple, showing 4.)/
\end{proof}
...
\begin{proof}
	...
	As both $A_0$ and $A'_0$ are three-dimensional over $K$ we conclude the map is then surjective.
\end{proof}
...
\begin{proposition}
	The quadratic space $(\qa{a}{b}, B)$ has an orthogonal basis $\{1, i, j, k\}$ and is isomorphic to the quadratic space $\langle 1, -a, -b, ab \rangle$.
\end{proposition}
...
\begin{proof}
	...
	so $w,z$ are orthogonal if and only if they anti-commute, hence $i,j,k$ are also mutually orthogonal so $\{1,i,j,k\}$ is an orthogonal basis for $B$. 
	This orthogonal basis then gives us the desired isomorphism of quadratic space as we have 
	\[B(1,1) = 1, \quad B(i,i) = -i^2 = -a, \quad\quad B(j,j) = -j^2 = -b, \quad \quad B(k,k) = -k^2 = ab\]
\end{proof}

\begin{remark}
	From the diagonalization above, if $x = \alpha + \beta i + cd + dk \in \qa{a}{b}$  then 
	\[N(x) = \alpha^2 - a\beta^2 - b c^2 + d^2ab\]
\end{remark}

As is the case with with the usual quaternions over the reals, we see that the norm is multiplicative, and so gives us a condition for invertibility, and even mroe an explicit description of its inerse when it exists.
...
Then note that $\phi(\beta) \in A'_0$ by \Cref{coroll:pure-quaternions} so
\[\alpha - \phi(\beta) = \overline{a + \phi(\beta)} = \overline{\phi(x)}\]
as claimed. 
Now $\phi$ then defines an isomorphism of quadratic spaces as 
\[B'(\phi(x),\phi(x)) = N(\phi(x)) = \phi(x) \overline{\phi(x)} = \phi(x) \phi(\overline{x}) = \phi(x \overline{x}) = \phi(N(x)) = N(x) = B(x,x)\]

Conversely, assume that $A$ and $A'$ are isomorphic as quadratic spaces. 
Then by \Cref{coroll:alg-witt-cancel} we have an isomorphism $\varphi: A_0 \to A'_0$ as quadratic spaces. 
Then note that 
...
therefore $\varphi(i)$ and $\varphi(j)$ generate $A_0$ over $K$, and satisfy the same relations defining $\qa{a}{b}$ which establishes an isomorphism.
\end{proof}

\subsection{The Hilbert Symbol}

We now define the Hilbert symbol, and show how it determine what will be called the splitting of a quaternion algebra.
...
\begin{proof}
	...
	Since $K_b = 1$ we have $N K_B^{\times} = N K^{\times} = K^{\times}$ which contains $a$, showing this particular case. \newline
	...
	So if $a$ lies in $N K_b^{\times}$ we have $a = x^2 - by^2$ then $(x,1,y)$ is a solution to $Z^2 -aX_1^2 -bX_2^2 = 0$ so $(a,b) = 1$.
...
\begin{theorem}\label{theo:splitting}
	The following conditions for the quaternion algebra $A = \qa{a}{b}$ are equivalent, in which case we say that $A$ \textbf{splits} over $K$ 	
	\begin{enumerate}
		\item $A \cong \qa{-1}{1}$.
...
\begin{proof}
	$1. \to 2.$, as if $A \cong \qa{-1}{1}$ we know  that $A \cong \mb{M}_2(K)$ from $\Cref{prop:basic-facts}$ which is not a divison algebra as it has zero-divisors. \newline
	...
	\end{align*}
	the last implication from our cancellation law in \Cref{coroll:alg-witt-cancel}. 
	...
	Given our assumption we have $a$ lies in $N K_b^{\times}$ from \Cref{prop:hilbert-norm}.
...
$\mc{O}_K$ will denote its corresponding discrete valuation ring with maximal ideal $\mk{m}$ and residue field $k$, and $\pi$ a choice of an uniformizer.
\newline 

	\indent If $f^{(i)} \in \mc{O}_K[X_1, \dots, X_n]$ are are all homogeneous polynomials (say a quadratic form), note any nonzero solutions in $(\mc{O}_K)^m$ are defined up to a scalar. 
...
This allows us to lift solutions mod $\m^n$ in many ways as we describe using the following variaton of Hensel's lemma.
...
\end{theorem}
\begin{proof}
	If $m = 1$ we easily reduce to our previous \Cref{lemm:hensel}. 
	Letting $x_0 = x$, we obtain an $x_1$ from $x$ and in general $x_i, i > 1$ such that $f(x_i) \equiv 0 \pmod{\m^{n+i}}$ amd $y = x \pmod{\m^{n-k + i}}$.
...
\subsection{Quadratic Forms Over Finite Extensions of $\Q_p$}

In this section we establish just enough facts about quadratic forms over the local fields $K$ we worked with in the previous section to be able to give a proof of the Hasse-Minkowski Theorem afterwards.
A common theme as follows is using our varation of Hensel's lemma to lift solutions from the finite fields $\mk{O}_K/\mk{m}$. 
...
\begin{proof}
	...
	By our previous \Cref{prop:char-not-2-lift} it suffices to show that the rank $n$ quadratic form $\overline{f} = \overline{d}_1 X_1^2 + \dots \overline{d}_n X_n^2$ over $k$ is isotropic for $n \geq 3$, which follows from our results on quadratic forms of finite fields. (\Cref{finite-field-isotropic}).
\end{proof}


When char $k = 2$ the argument for lifting solutions in \Cref{prop:char-not-2-lift} doesn't work as $2 = 0$ so the partials above are all zero.
...
We will now begin establish a short description of the squares over $K$ whose residue field is not of characteristic 2.
We use the following theorem, which also gives us some information about the characteristic 2 case. 

\begin{theorem}\label{prev}
...
\end{theorem}
\begin{proof}
	We may apply \Cref{theo:hensel} to the polynomial $X^2 - (1 + 4\pi \alpha)$ with $x = 1, k = v(2), n = v(4 \pi \alpha)$ 
	...
	Finally, the latter equality says $v(c) = 0$ so $c$ is a unit, which applying the above to any element of $mk{m}$ gives us $1 + 4 \mk{m} \subset U^2$.
\end{proof}

\begin{theorem}
	If $\text{char} (k) \neq 2$, express any $y \in \mc{O}_K$ as $\pi^{v(y)}x$ where $x$ is a unit. 
	Then $y$ is a square if and only if $x \pmod {m}$ is a square in $k$ and $v(y)$ is even.
\end{theorem}

\begin{proof}
	If $y = b^2$ is a square, it must have even valuation, and $x$ must be a square as well. 
	And reducing $x$ mod $\mk{m}$ shows $x \pmod{m}$ must be a square in $k$.
	...
	As $\text{char}(k) \neq 2$ we have 4 is invertible in $k$, so every such $x$ can be expressed as $1 = 4 \pi a$, $a \in \mc{O}_K$ so by our previous theorem $x = c^2$ for some unit $c \in \mc{O}_K$.
	Then if $v(y)$ is even we have $y = (\pi^{(v(y)/2} c))^2$ is a square as claimed.

\end{proof}

\begin{corollary}
	If $\text{char}(k) \neq 2$, then  $|K^{\times}/(K^{\times})^2| = 4$, with representatives $1, u, \pi, u\pi$ where $u \pmod{\mk{m}}$ is not a square in $k$. 
\end{corollary}

\begin{remark}
	In general one can show that $|K^{\times}/(K^{\times})^2| = 4|k|^e$, where $e = v(2)$, see [\cite{ATOQF}, VI, 2.22]
	For $\text{char}(k) \neq 2$ this reduces to our previous result, and for $\text{char}(k) = 2$ we have $|K^{\times}/(K^{\times})^2| = 2^{n+2}$ for $n$ the degree of the extension $K/\Q_2$. 
\end{remark}
...
\end{corollary}
\begin{proof}
	This says $c/d \equiv 1 \bmod{4 \pi}$, now apply \Cref{prev}.
\end{proof}
...
\begin{notation}
	Throughout this subsection, $F \supset \Q$ will denote a number field, and $V$ the set of equivalence classes of norms (both archimedean and non-archimedean) on $F$.
\end{notation}


Our proof of the Hasse-Minkowksi Theorem over number fields won't be entirely self-contained and we will need two major results which we state below without proof.
...
The second theorem may be viewed as a special case of the Albert-Brauer-Hasse-Noether theorem, which is similarly stated but more generally for central simple algebras over $F$.
...
\begin{proof}
	From the embeddings $F \hookrightarrow F_v$ for each $v \in V$, then if $F$ is isotropic, the same element over $F$ shows it is isotropic over the $F_v$. \newline

	\indent To show the conditions are sufficient, we argue depending on the rank $n$ of $f$.
	We may always represent an  $n$-ary form $f$ as $X_1^2 + a_2X_2^2 + \dots + a_n X_n^2$ (corresponding to the quadratic space $\langle 1, a_2, \dots, a_n \rangle$) as if $f = a_1X_1^2 + \dots a_n X_n^2$ is a diagonal form, $f$ is isotropic if and only if $a_1 f$ is isotropic, and the latter is equivalent to $X_1^2 + a_2X_2^2 + \dots + a_n X_n^2$.
...
So if $-a = b^2$ in $F$ and we see $(b,1)$ shows $f$ is indeed isotropic over $F$. \newline
...
This is the quadratic form associated to $(\frac{a,b}{L_v})$, and it is isotropic (as $L_v$ is either $\R, \C$ or is an extension of some $F_v$ where we know it is already isotropic by assumption) so that $(\frac{a,b}{L_v})$ splits (by \Cref{theo:splitting} again).
...
Then there is a finite subset $S \subset V$ such that $h$ is isotropic on $F_j$ for $j \in V - S$ by \Cref{lifting-up} (we can take $S$ to be the equivalence classes consisting of the archimedean norms, those primes over 2, and any primes dividing a coefficient of $h$).
For any $s \in S$, as $f = g - h$ is isotropic in $F_s$ by assumption there is some $x_s \in F_s$ such that $g,h$ both represent $s$. 
By the Approximation Theorem (\cite{ANT}, II.3.4) we can find $x \in F$ such that $||x -x_s|| < \epsilon_s$ (here we use the multiplicative valuation) for all $s$ and arbitrarily small $\epsilon_s$.
...
then from the previous constructions, by constraining the $\epsilon$'s small enough we can find $x_1, x_2 \in F$ such that $||g(x_1, x_2) - x_s|| < \epsilon_s$ for all $s$ so that $g(x_1, x_2)/x_s \in (F_s^{\times})^2$.
To show that $f = g -h$ is isotropic it then suffices to show that $h$ represents $g(x_1, x_2)$.
For each $s \in S$ we know $h$ represents $x_s$, since $g(x_1, x_2)/x_s \in  (F_s^{\times})^2$ then $h$ also represents $g(x_1, x_2)$.
For $v \in V-S$, $h$ is isotropic so represents all of $F_v^{\times}$, in particular $g(x_1, x_2)$ as well (by \Cref{prop:isotropic-hyperbolic}).
...
\begin{proof}
	Apply the Hasse-Minkowksi Theorem to the quadratic form $f - aZ^2$.
\end{proof}
...
The counterexample to the local-global principle we mentioned in the introduction for higher degreee equations is Selmer's equation
\[f(X,Y,Z) = 3X^3 + 4Y^3 + 5Z^3\]
which we will show has nontrivial solutions in each $\Q_p$ (and $\R$), but no nontrivial solution over $\Q$.
...
\begin{proof}
	...
	Indeed, this is because we have $\mc{O}_{Q(\alpha)}/A_p \cong \prod_{i =1}^n \Z/p_i^{n_i} \Z$ for primes $p_i$ dividng the index and corresponding $n_i$, then as tensor product always commutes with quotients, 
...
As we verified the ring of integers is $\Z[\alpha] \cong \Z[x]/(x^3 - 6)$, the splitting of primes over $p$ depends on the splitting of the polynomial $x^3 - 6$ in the fibers $\mb{F}_p[x]/(x^3 - 6)$.
...
Now in $\mb{F}_5[x]$ we have that $x^3 - 6 = x^3 - 1$ factors into irreducibles as $(x-1)(x^2 + x + 1)$, so there are two primes $\mk{p}_5$ and $\mk{p}_{25}$ of $\Q(\alpha)$ with corresponding norms, lying over 5.
...
Since $|N_{Q}^{Q(\alpha)}(\alpha - 2)| = |-2| = 2$, $|N_{Q}^{Q(\alpha)}(\alpha - 1)| = 5$, and  $|N_{Q}^{Q(\alpha)}(\alpha - 1)| = |-5| = 5$ are also the norms of their principal ideals, we have $\mk{p}_2 = (\alpha - 2)$ and $\mk{p}_5 = (\alpha - 1)$.
...
\begin{proof}
	...
	Hence the only element in our class group are the principal ideals as claimed.
\end{proof}
...
Similarly arguing for the primes 3 and 5 we also conclude that \textbf{$X,Z$  are not divisible by 3} and \textbf{$X,Y$ are not divisible by $5$}. \newline
...
We claim that for some ideal $\mk{q}$ that the principal ideal $(X + Y\alpha)$ can be expressed as 
\begin{equation}\label{eq3}
	(X + Y\alpha) = \mk{p}_2 \mk{p}_5 \mk{q}^3 = (\alpha - 2)(\alpha -1) \mk{q}^3
\end{equation}
Examining \Cref{eq2}, we know that $(10) = (2)(5) \mk{p}_2^3 \mk{p}_5 \mk{p}_{25}$ divides $(X+Y\alpha )(X^2 - XY\alpha + Y^2 \alpha^2)$.\newline
\indent Since $(\alpha)^3 = (6) = \mk{p}_2^3 (3)$ we have $\mk{p}_2 | \alpha$, and by assumption $X$ is even so $\mk{p}_2 | X$ as well, so $\mk{p}_2 | (X + Y\alpha)$, and $\mk{p}_2^2 | (X^2 - XY\alpha + Y^2 \alpha^2)$. \newline
\indent Looking at $X^3 + 6Y^3 = 10Z^3$ mod 5 we have $X^3  \equiv -Y^3 \pmod{5}$, and as mentioned, since $5 \neq 1 \pmod{3}$ cubing is bijective on $\Z/5\Z$ so this implies that $X \equiv -Y \pmod{5}$.
Since $\mk{p}_5 = (\alpha - 1)$ we also have $\alpha \equiv 1$ in $\Z[\alpha]/\mk{p}_5 \cong \Z/5\Z$.
Hence we have in $\Z[A]/\mk{p}_5$ that
\[X + Y\alpha \equiv X + Y \equiv 0 \pmod{5} \]
so $\mk{p}_5 | X + Y\alpha $.
If $\mk{p}_{25}$ also dividing $(X + Y\alpha)$ then $5 = \mk{p}_5 \mk{p}_{25}$ would as well, implying that $X,Y$ are divisible by 5 which contradicts one of our original assumptions.
So $\mk{p}_{25} | (X^2 - XY\alpha + Y^2 \alpha^2)$. \newline
\indent We have established that 
\[(X + Y\alpha) = \mk{p}_2 \mk{p}_5 \mk{q}, \quad (X^2 - XY\alpha + Y^2 \alpha^2) = \mk{p}_2^2 \mk{p}_{25}\mk{q}' \]
for ideals $\mk{q}$ and $\mk{q}'$ such that $\mk{q}\mk{q}' = (Z)^3$.
Since $Z$ was assumed odd we know that $\mk{p}_2$ does not divide $Z$, and hence doesn't divide $\mk{q}$ and $\mk{q}'$ as well.
We claim that, other than $\mk{p}_2$, $(X + Y\alpha)$ and $(X^2 - XY\alpha + Y^2 \alpha^2)$ share no common prime factors, so by unique factorization of ideals we would have $\mk{q} = \alpha^3, \mk{q}' = \alpha'^3$ are cubes, which would establish our claim in \Cref{eq3}.\newline

\indent If there were some $\mk{p} \neq \mk{p}_2$ such that $\mk{p} | (X + Y\alpha)$ and $\mk{p}|(X^2 - XY\alpha + Y^2 \alpha^2)$ it would also divide $(X + Y\alpha)^2 - (X^2 - XY\alpha + Y^2 \alpha^2) = (3)(X)(Y)(\alpha)$.
We'll show $\mk{p}$ cannot divide $(3)(X)(Y)(\alpha)$.\newline
\indent First note that $\mk{p} \nmid (3)$, as if it did divide $(3)$ it doesn't lie over 2 or 5 and so $\mk{p} \nmid (10)$, which from $\Cref{eq2}$ we have $\mk{p}|Z^3$, but one of our original observations was that $Z$ was not divisible by 3, hence relatively prime to it. \newline
\indent If $\mk{p} | (X)$, then $\mk{p}|(Y)(\alpha)$ as $\mk{p}|(X + Y\alpha)$, but as $X$ and $Y$ are relatively prime we would have $\mk{p}|\alpha$ (The same argument shows that $\mk{p}\nmid(Y)$).
However, $(\alpha)^2 = (6) = (2)(3)$, as we already established $\mk{p}\nmid (3)$ we would have $\mk{p} = \mk{p}_2$ a contradiction, so $\mk{p} \nmid (X)$, so such an $\mk{p}$ cannot exist, which as remarked implies our claim in $\Cref{eq3}$. \newline

\indent As a result, for some unit $\mu$ we have an equality
\[X + Y\alpha = (\alpha - 2)(\alpha -1) \mk{q}^3 \mu\]
Absorbing any cube factor of $\mu$ in $\mk{q}^3$, we may assume its cube free so 
\[\mu = (1 - 6\alpha + 3\alpha^2)^k = \frac{((2 - \alpha)^{3})^k}{2^k}\]
substituting this back in (absorbing (($2 - \alpha)^{3})^k$ into $\mk{q}^3$) and multiplying through by $2^k$ we have
\begin{equation}\label{eq4}
2^kX + 2^kY\alpha = (\alpha - 2)(\alpha - 1) \mk{q}'^3
\end{equation}
for some ideal $\mk{q}'^3$.
Letting $\mk{q} = (A + B\alpha + C\alpha^2)$ for $A,B,C \in \Z$ (we know $\mk{q}$ is principal by 
\Cref{PID}), upon expanding out the above equation and equating the coefficients of $\alpha^2$ we obtain
\[0 = A^3 + 6B^3 + 36C^3 + 36ABC -9(A^2B + 6AC^2 + 6B^2C) + 6(AB^2 + A^2C + 6BC^2)\]
Every factor other than $A^3$ above is divisible by 3, so looking mod 3 we must have $A$ is divisible by $3$.
Then $9$ divides every term above except $6B^3$ so we similarly obtain that $3$ divides $B$.
Once again, we see that 27 divides every factor above except $36C^2$, so $3$ then divides $C$.
By the homogeneity of the above equation, $(A/3, B/3, C/3)$ is another valid integer solution, but we may indefinitely continue this proces so necessarily $A = B = C = 0$.
But by $\Cref{eq4}$ this implies $(X + Y\alpha) = 0$ a contradiction.
So there cannot be any nontrivial solutions to $X^3 + 6Y^3 = 10Z^3$ and hence no nontrivial solutions for Selmer's equation over $\Q$.

\begin{remark}
	Selmer's equation above is not unique in this regard, there are other such cubics satisfying the same property such as $x^3 + 3y^3 + 22z^3$.
\end{remark}
