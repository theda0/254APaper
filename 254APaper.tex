...
$\mc{O}_K$ willdenote its corresponding discrete valuation ring with maximal ideal $\mk{m}$ and residue field $k$, and $\pi$ a choice of an uniformizer.
\newline 
...
substituting this back in (absorbing $2 - \alpha)^{3})^k$ into $\mk{q}^3$) and multiplying by $2^k$ we have
\begin{equation}\label{eq4}
2^kX + 2^kY\alpha = (\alpha - 2)(\alpha - 1) \mk{q}'^3
\end{equation}

for some ideal $\mk{q}'^3$.
Letting $\mk{q} = A + B\alpha + C\alpha^2$ for $A,B,C \in \Z$ (we know its principal by 
...
But be $\Cref{eq4}$ this implies $(X + Y\alpha) = 0$ a contradiction.
